...
let get_default_ref _t (repo_id : Repo_id.t) =
  ...
  Lwt.return { Commit_id.repo = repo_id
             ...
             ; hash = c.commit_id
             ; committed_date = Gitlab_json.Date.unwrap c.commit_created_at }
...
let parse_ref ~repo ~prefix (branch : Gitlab_t.branch_full) : Commit_id.t =
  ...
  let committed_date = Gitlab_json.Date.unwrap branch.branch_full_commit.commit_committed_date in
...
let parse_merge_request ~repo (mr : Gitlab_t.merge_request) : Commit_id.t =
  ...
  let committed_date = Gitlab_json.Date.unwrap mr.merge_request_updated_at in
